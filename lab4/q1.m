...
%     n = size(A, 1);
% 
%     Копируем исходную матрицу и вектор
%     new_A = A;
%     new_b = b;
% 
%     Делаем матрицу диагонально доминирующей
%     for i = 1:n
%         row_sum = sum(abs(A(i, :))) - abs(A(i, i)); % Сумма вне диагонали
%         if abs(A(i, i)) <= row_sum
%             Увеличиваем диагональный элемент
%             new_A(i, i) = row_sum + 1;
%         end
%     end
% end
...
% --- Метод Якоби ---
[x_jacobi, iter_jacobi] = jacobi_method(new_A, new_b, x0, accuracy);
fprintf('Метод Якоби сошёлся за %d итераций.\n', iter_jacobi);
disp('Решение методом Якоби:');
disp(x_jacobi);

% --- Метод Зейделя ---
[x_zeidel, iter_zeidel] = seidel_method(new_A, new_b, x0, accuracy);
...
[x_simple, iter_simple] = simple_iterations(new_A, new_b, x0, tau, accuracy);
...
x_linsolve = linsolve(new_A, new_b);
...
function [new_A, new_b] = make_diagonally_dominant(A, b)
    ...
    D(D == 0) = 1
    % Вычислим матрицу B = D * inv(A)
    new_A = D; % Эквивалентно B = D * inv(A)
