...
d6f = @(x) a * b^6 * exp(b * x) - 120 * c ./ x.^6; % n + 2 (для 13.28)
...
% Степень полинома
n = 4;
% Порядок производной (c 0 индекса)
i = 2;
...
% Вычисляем Π'5(x) и его производную
P5 = @(x) prod(x - xi);  % Π'5(x)
dP5 = @(x) sum(arrayfun(@(k) prod(x - xi([1:k-1, k+1:end])), 1:length(xi)));
...
fprintf("Оценка погрешности f'(x12) по интеполяционной формуле (13.28) = %.4f\n", error13_28);
