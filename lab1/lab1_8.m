...
% result = cell(size(structMatrix));

% % Объединяем символы и числа с учетом условий
% for i = 1:size(structMatrix, 1)
%     for j = 1:size(structMatrix, 2)
%         if structMatrix(i, j) == 0
%             % Если значение 0, не выводим ничего
%             result{i, j} = ' '; % Используем {} для присвоения
%         elseif structMatrix(i, j) == 1
%             % Если значение 1, выводим только символ
%             result{i, j} = nameReag{j}; % Используем {} для присвоения
%         else
%             % Иначе выводим символ и число
%             result{i, j} = [nameReag{j}, num2str(structMatrix(i, j))]; % Используем {} для присвоения
%         end
%     end
% end
% disp(result);
